...
describe('conversion error', () => {
  it('should solve equations', async () => {
    const equations = await readFromFile('../part-1/puzzle-sample.txt');
    const e = equations[1];
    ...
    expect(tokens).toEqual(459236326669);
  });

  it.skip('should solve the puzzle', async () => {
    const equations = await readFromFile('../part-1/puzzle-input.txt');
...
async function readFromFile(filename: string): Promise<ButtonEquation[]>{
  ...
  for await (const line of rl) {
    ...
    if (line.startsWith('Prize')) {
      ...
      equation.c = Number(x) + 10000000000000;
      equation.f = Number(y) + 10000000000000;
